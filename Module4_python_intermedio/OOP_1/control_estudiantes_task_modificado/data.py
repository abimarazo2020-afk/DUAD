...
def import_CSV_file(file_path, list_dic): #esta funcio lee el csv, crea los estudiantes y los mete en una lista
    try:
        ...
        with open(file_path, 'r', newline='', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                student = Student(
                    row['student_name'],  #se usa row porque viene de csv
                    row['student_section'],
                    int(row['spanish_note']), #se usa int porque viene como texto 
                    int(row['scient_note']),
                    int(row['social_note']),
                    int(row['english_note'])
                )
